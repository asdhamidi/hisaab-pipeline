    ...
    try:
        ...
        df_activity_metric = (
            ...
            .withColumn(
                "time_of_the_day",
                F.when(
                    F.hour(F.col("activity_created_at")) < 12, "Morning"
                ).when(
                    (F.hour(F.col("activity_created_at")) >= 12) &
                    (F.hour(F.col("activity_created_at")) < 18), "Afternoon"
                ).otherwise("Night")
            )
            .groupBy(
                ["time_of_the_day", "a.username"]
            )
            .agg(
            ...
            .withColumn("modification_ratio", F.col("modification_count") / F.col("activity_count"))
            .withColumn("created_at", F.expr("current_timestamp()"))
            .withColumn("created_by", F.expr("current_user()"))
        )

        write_data_to_table(df_activity_metric, "gold.gold_user_activities")
